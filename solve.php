...
class Travel
{
    public static function fetch()
    {
        ...
        if ($data === false) die("Failed to fetch travel data from API.");
        
        $decoded_data = json_decode($data, true);
        
        if ($decoded_data === null) die("Failed to decode travel data: " . json_last_error_msg());
...
class Company
{
    public static function fetch()
    {
        ...
        $data = @file_get_contents($url);
        if ($data === false) die("Failed to fetch company data from API.");

        $decoded_data = json_decode($data, true);

        if ($decoded_data === null) die("Failed to decode company data: " . json_last_error_msg());
